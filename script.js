...
function openDetail(id, indexInFiltered) {
  ...
  document.getElementById("detail-content").innerHTML =
    ...
    '<button class="btn-copy" onclick="copyFiche(' + id + ')" title="Copier la définition">📋 Copier</button>' +
    '<div class="panel-nav">' +
...
function copyFiche(id) {
  ...
  const lines = [
    ...
    "Définition : " + f.def,
    "",
    f.points ? "Points clés :\n" + f.points.map(p => "• " + p).join("\n") : "",
    "",
    f.piege ? "⚠️ Piège : " + f.piege : "",
    f.retenir ? "✅ À retenir : " + f.retenir : "",
  ].filter(Boolean).join("\n");
